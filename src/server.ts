...
function sendBodyError(res:ServerResponse,error:unknown){if(error instanceof RequestBodyTooLargeError)sendJson(res,413,{error:error.message});else sendJson(res,400,{error:"Invalid JSON"});}
function safeSend(socket:WebSocket,event:unknown){if(socket.readyState===WebSocket.OPEN)socket.send(JSON.stringify(event));}
function validateTwilioWebhook(config:Config,req:IncomingMessage,params:URLSearchParams){if(!config.twilioAuthToken)return true;const signature=req.headers["x-twilio-signature"];if(typeof signature!=="string")return false;return twilio.validateRequest(config.twilioAuthToken,signature,`${config.publicBaseUrl}${req.url??"/voice/incoming"}`,Object.fromEntries(params.entries()));}
...
export function createMasteryServer(config:Config){
 ...
 const server=createServer(async(req,res)=>{const url=new URL(req.url??"/","http://localhost");const path=url.pathname;
  ...
  if(req.method==="POST"&&path==="/linq/webhook"){linqWebhookCount+=1;lastLinqWebhookAt=new Date().toISOString();if(!config.linqApiToken||!config.linqWebhookSecret)return sendJson(res,503,{error:"Linq webhook verification is not configured"});let rawBody:string;try{rawBody=await readBody(req);}catch(error){return sendBodyError(res,error);}if(!verifyLinqSignature(rawBody,req.headers,config.linqWebhookSecret))return sendJson(res,401,{error:"Invalid Linq signature"});let body:JsonObject;try{body=JSON.parse(rawBody);}catch{return sendJson(res,400,{error:"Invalid JSON"});}const message=parseLinqMessage(body,req.headers);if(message)lastParsedLinqMessageAt=new Date().toISOString();sendJson(res,200,{received:true,release:RELEASE});if(message)processLinqMessage(config,message).catch(error=>console.error("Linq message failed:",error));return;}
