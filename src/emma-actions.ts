...
import { updateStudioState } from "./studio-store.js";
...
export async function detectStudioAction(config: Config, text: string, context: Record<string, unknown>, fetchImplementation: typeof fetch = fetch): Promise<EmmaStudioAction | null> {
  ...
  const raw=String(result.output_text??"").trim();
  if(raw==="NONE")return null;
  const match=raw.match(/^(vision|mission|weeklyTheme)\|(.+)$/s);
  if(!match)return null;
  return {field:match[1] as EmmaStudioAction["field"],value:match[2].trim()};
}

export async function applyStudioAction(config: Config, _phone: string, action: EmmaStudioAction) {
  return updateStudioState(config, { [action.field]: action.value });
}
